...
receipt_metrics AS (
    SELECT
        ...
        DATEDIFF(day, MAX(DATE), TO_DATE('${as_of_date}')) / 7.0 / NULLIF(MAX_BY(TRANSACTION_QUANTITY_2, DATE), 0) AS "Quantity Received, Average Since Last Receipt"
    FROM receipt_moves
    GROUP BY PART_NUMBER
),
...
alternate_receipt_metrics AS (
    SELECT
        ...
        DATEDIFF(day, arl.DATE, TO_DATE('${as_of_date}')) / 7.0 / NULLIF(arl.TRANSACTION_QUANTITY_2, 0) AS "Quantity Received, Average Since Last Receipt with alternates"
    FROM alternate_part_bridge ab
...
inventory_metrics AS (
    SELECT
        ...
        SUM(CASE WHEN lcm.MRP_CATEGORY = 'Warehouse' THEN inv.${inventory_quantity_column} ELSE 0 END) AS "Current On-Hand Quantity",
